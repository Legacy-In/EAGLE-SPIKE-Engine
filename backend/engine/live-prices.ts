...
// In-memory cache for live exchange tickers (TTL = 3000ms)
...
const priceCache: Map<string, CachedQuote> = new Map(); // key: "EXCHANGE:SYMBOL"

export class LivePriceService {
  /**
   * Updates cache from live WebSocket ticker event
   */
  ...
  /**
   * Fetches the latest verified perpetual/futures price for a symbol on a specific exchange
   */
  public static async getLivePrice(
    ...
    // 1. Check fresh in-memory cache (< 4000ms old)
  ...
  /**
   * Fetches from exchange-specific perpetual contract endpoint
   */
  ...
  /**
   * Classifies data quality based on quote age
   */
  ...
  /**
   * Calculates deterministic ROI strictly between ENTRY and CURRENT
  ...
  /**
   * Dynamic institutional price formatter for micro and macro assets
   * PEPE (0.0000044) -> "$0.00000440"
   * BTC (65432.1)    -> "$65,432.10"
   */
