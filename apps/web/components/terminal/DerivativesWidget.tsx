...
import { BtcPositioningPanel } from './BtcPositioningPanel';
...
export const DerivativesWidget: React.FC<DerivativesWidgetProps> = ({ derivatives }) => {
  ...
  const divergenceLabel = positioning?.interpretation?.state
    ? positioning.interpretation.state.replace(/_/g, ' ')
    : derivatives?.oiPriceDivergence || 'ORGANIC EXPANSION';
  ...
  return (
    <div className="space-y-3">
      <div className="bg-sigma-surface1 border border-sigma-border rounded-lg p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5 font-mono text-[10px]">
            <span className="text-sigma-textDark">STATE:</span>
            <span className="text-sigma-green font-bold uppercase">{divergenceLabel}</span>
          </div>
        </div>
      ...
      {/* Core BTC Price x OI Positioning Engine Panel */}
      <BtcPositioningPanel positioning={positioning} derivatives={derivatives} />
    </div>
  );
};
