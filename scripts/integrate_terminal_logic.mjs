...
console.log('🦅 Injecting Production Spike Intelligence JavaScript Engine into eagle-flash.html...');

const filePath = path.join(process.cwd(), 'apps', 'web', 'public', 'eagle-flash.html');
let html = fs.readFileSync(filePath, 'utf-8');

// ═════════════════════════════════════════════════════════════════════════
// 1. Helper Functions (Sparklines, Badges, Popovers)
// ═════════════════════════════════════════════════════════════════════════
const helperFunctionsJs = `
    // ═════════════════════════════════════════════════════════════════════════
    // SPIKE INTELLIGENCE UI HELPERS (Sparklines, Badges, Explainable Scoring)
    // ═════════════════════════════════════════════════════════════════════════

    function generateSparklineSvg(ret5m, ret15m, ret24h) {
      const isBull = (ret5m || ret24h) >= 0;
      const stroke = isBull ? '#00E599' : '#FF4757';
      // 5-point normalized trajectory
      const p0 = 9;
      const p1 = Math.max(2, Math.min(16, 9 - (ret24h || 0) * 0.2));
      const p2 = Math.max(2, Math.min(16, 9 - (ret15m || 0) * 0.8));
      const p3 = Math.max(2, Math.min(16, 9 - (ret5m || 0) * 1.5));
...
// Inject helper functions before recalculateAllScores
if (!html.includes('generateSparklineSvg')) {
  html = html.replace('function recalculateAllScores()', helperFunctionsJs + '\n    function recalculateAllScores()');
  console.log('✅ Injected helper functions (Sparklines, Badges, Explainable Scoring).');
}

// ═════════════════════════════════════════════════════════════════════════
// 2. Upgrade recalculateAllScores()
// ═════════════════════════════════════════════════════════════════════════
const newRecalculateAllScoresJs = `    function recalculateAllScores() {
...
// Replace old recalculateAllScores
const oldRecalcRegex = /function recalculateAllScores\(\)[\s\S]*?\n    \}/i;
html = html.replace(oldRecalcRegex, newRecalculateAllScoresJs);
console.log('✅ Upgraded recalculateAllScores with 8-factor score, 9-state lifecycle, and type/quality engines.');

// ═════════════════════════════════════════════════════════════════════════
// 3. Upgrade renderScannerTable() with Primary Columns & Sparklines
// ═════════════════════════════════════════════════════════════════════════
const newRenderScannerTableJs = `    function renderScannerTable() {
      ...
      list.slice(0, 100).forEach((s, idx) => {
        ...
        html += \`
          <tr onclick="openSymbolDetail('\${s.symbol}')" id="row-\${s.symbol}">
            ...
            <td class="tabular \${isPos5m ? 'c-emerald' : 'c-red'} font-bold">\${isPos5m ? '+' : ''}\${s.returns5m || 0}%</td>
            ...
            <td class="tabular \${isPosTaker ? 'c-emerald' : 'c-red'} font-bold">\${isPosTaker ? '+' : ''}\${(s.takerImbalance || 0).toFixed(0)}%</td>
            <td>
        ...
        // Mobile Card (Key metrics first)
        mHtml += \`
          <div class="m-card" onclick="openSymbolDetail('\${s.symbol}')">
...
// Replace old renderScannerTable
const oldRenderTableRegex = /function renderScannerTable\(\)[\s\S]*?\n    \}/i;
html = html.replace(oldRenderTableRegex, newRenderScannerTableJs);
console.log('✅ Upgraded renderScannerTable with primary visible columns and SVG sparklines.');

// ═════════════════════════════════════════════════════════════════════════
// 4. Upgrade openSymbolDetail() with Full Intelligence Sections
// ═════════════════════════════════════════════════════════════════════════
const newOpenSymbolDetailJs = `    async function openSymbolDetail(sym) {
...
// Replace old openSymbolDetail
const oldOpenDetailRegex = /async function openSymbolDetail\(sym\)[\s\S]*?\n    \}/i;
html = html.replace(oldOpenDetailRegex, newOpenSymbolDetailJs);
console.log('✅ Upgraded openSymbolDetail with full Section 30 intelligence sections.');

// ═════════════════════════════════════════════════════════════════════════
// 5. Upgrade renderKPIs() with BTC Regime Pill & Freshness
// ═════════════════════════════════════════════════════════════════════════
const newRenderKPIsJs = `    function renderKPIs() {
...
// Replace old renderKPIs
const oldRenderKPIsRegex = /function renderKPIs\(\)[\s\S]*?\n    \}/i;
html = html.replace(oldRenderKPIsRegex, newRenderKPIsJs);
console.log('✅ Upgraded renderKPIs with BTC Regime pill and freshness.');

// ═════════════════════════════════════════════════════════════════════════
// 6. Test Syntax with vm.Script Before Saving
// ═════════════════════════════════════════════════════════════════════════
const scriptMatch = html.match(/<!-- APPLICATION JAVASCRIPT LOGIC -->\s*<script>([\s\S]*?)<\/script>\s*<\/body>/i);
if (!scriptMatch) {
  console.error('❌ Could not locate APPLICATION JAVASCRIPT LOGIC script tag!');
  process.exit(1);
}

try {
  new vm.Script(scriptMatch[1]);
  console.log('✅ JavaScript syntax verified with zero errors.');
} catch (err) {
